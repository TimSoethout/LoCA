...
trait TransactionLogic extends RebelSpecification[Transaction.type] {
  ...
  override def syncOperations(domainEvent: RDomainEvent)(data: RData): RebelCheck[Set[SyncOperation]] =
    domainEvent.specEvent match {
      case Book                                =>
        data.map(tData => {
          RebelCheck.success(Set[SyncOperation](
            TwoPhaseCommit.SyncAction(ParticipantId(UniqueId.apply(tData.from.toString)),
              ShardingContactPoint(Account, tData.from),
              RebelDomainEvent(Withdraw(tData.amount))),
            TwoPhaseCommit.SyncAction(ParticipantId(UniqueId.apply(tData.to.toString)),
              ShardingContactPoint(Account, tData.to),
              RebelDomainEvent(Deposit(tData.amount)))
          ))
        }).getOrElse {
          RebelConditionCheck.failure(GenericRebelError("State not initialised"))
        }
      case Start(id, amount, from, to, bookOn) =>
        RebelCheck.success(Set[SyncOperation](
          ...
          TwoPhaseCommit.SyncAction.syncInitialized(
            ParticipantId(UniqueId.apply(from.toString)), domainEvent.transactionId, ShardingContactPoint(Account, from)),
          TwoPhaseCommit.SyncAction.syncInitialized(
            ParticipantId(UniqueId.apply(to.toString)), domainEvent.transactionId, ShardingContactPoint(Account, to))
        ))
    }
