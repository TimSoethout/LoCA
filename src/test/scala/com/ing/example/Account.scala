...
//  override lazy val initialState: Account.AccountState = New
  ...
  override def checkPreConditions(data: RData, now: DateTime): PartialFunction[AccountEvent, RebelConditionCheck] = {
    ...
    case _                                      => Validated.invalidNel(PreConditionFailed("precondition check not implemented."))
  }
  ...
  override def invariantsHold(data: RData): RebelConditionCheck = {
    ...
    if (data.exists { rdata =>
      ...
      Validated.invalidNel(InvariantFailed("Some invariant failed."))
    }
  }

  override def syncOperations(domainEvent: RDomainEvent)(data: RData): RebelCheck[Set[SyncOperation]] = RebelCheck.success(Set())
}
