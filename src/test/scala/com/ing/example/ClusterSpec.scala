...
class InMemClusterSpec extends IsolatedCluster(ConfigFactory.parseString("rebel.sync.max-transactions-in-progress = 1"))
  with ClusterSpec
...
class InMemClusterPsacSpec extends IsolatedCluster(ConfigFactory.parseString("rebel.sync.max-transactions-in-progress = 8"))
  with ClusterSpec
...
//    accounts ! (iban1, CheckPreconditions(tId, IsInitialized))
//    expectMsg(CheckPreconditionsResult(tId, RebelCheck.success(Set())))

    // TODO fails now because of Query never being released, except after timeout
  ...
  it should "also correctly fail transactions" in {
    ...
    atLeast(1, failed.rebelErrors.toList) should matchPattern { case SyncFailed(msg) if msg.contains("IsInitialized") => }
    ...
    atLeast(1, bookFailed.rebelErrors.toList) should matchPattern { case e: RebelError if e.toString.contains("Command Book not allowed when in state Init") => }
  }
