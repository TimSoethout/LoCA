...
object EndpointMessageLogger {
  ...
  def route(endpointMessageLoggerRef : ActorRef)
    ...
    path("diagram") {
      get {
        parameters('startTime.as[DateTime], 'endTime.as[DateTime], 'maxSize ? Int.MaxValue) {
...
trait LoggingInterceptor extends Actor with MessageInterceptor {
  ...
  val toggle: Boolean = context.system.settings.config.getBoolean("rebel.visualisation.enabled")
