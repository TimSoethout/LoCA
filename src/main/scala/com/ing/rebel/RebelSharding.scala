...
//    implicit val eventIsEvent : Spec#Event =:= spec.Event = implicitly[Spec#Event =:= spec.Event]
    ...
    val shardShardIdLookupCounter: CounterMetric = Kamon.counter(s"shardid-$name")
        ...
        extractShardId = { m: Msg =>
          ...
          val counter = m match {
            case ShardingEnvelope(key, RebelCommand(domainEvent))  =>
              shardShardIdLookupCounter.refine(("shardId", result), ("msgType", domainEvent.specEvent.getClass.getSimpleName))
            case ShardingEnvelope(key, mess) =>
              shardShardIdLookupCounter.refine(("shardId", result), ("msgType", mess.getClass.getSimpleName))
            case _                           => shardShardIdLookupCounter.refine(("shardId", result))
          }
          counter.increment()
          result
        }
