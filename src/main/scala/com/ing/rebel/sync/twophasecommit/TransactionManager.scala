...
class TransactionManager()(override implicit val domainEventClassTag: ClassTag[TransactionManager.ManagerEvent])
  extends LoggingPersistentFSM[TransactionManager.ManagerState, TransactionManager.ManagerData, TransactionManager.ManagerEvent]
    with ActAfterXTimeouts[TransactionManager.ManagerState, TransactionManager.ManagerData, TransactionManager.ManagerEvent]
    with PersistentFSM[TransactionManager.ManagerState, TransactionManager.ManagerData, TransactionManager.ManagerEvent]
...
//    with ActorVisualization
  ...
  startWith(Init, ManagerData(null, TransactionId(null), null, null, null))
  ...
  override def stateAfterMaxRetries: PartialFunction[(ManagerState, ManagerData), State] = {
    case (Wait, _) =>
      goto(Abort) applying
        AbortReasonsAdded(NonEmptyList.of(SyncFailed(s"TransactionManager waiting but not received all votes within $nextTimeoutAt"))) andThen
        globalAbort
  }
  ...
  override def applyEvent(domainEvent: ManagerEvent, currentData: ManagerData): ManagerData = {
    ...
    domainEvent match {
      case Initialized(requester, transactionId, participants) =>
        ...
        ManagerData(requester, transactionId, participants, participants.map(_.participantId).toSet, NonEmptyList.of(SyncFailed(s"Sync `$transactionId` failed")))
      ...
      case AbortReasonsAdded(reasons)                          =>
        currentData.copy(failureReasons = currentData.failureReasons.concatNel(reasons))
    }
  }

  def incrementKamonCounter(counterName: String): Unit = {
    if(!recoveryRunning) {
      Kamon.counter(counterName).increment()
    }
  }
