...
import com.ing.rebel.{QuerySyncEvent, RebelDomainEvent, RebelErrors, RebelState, RebelSyncEvent}
...
class TransactionParticipant[S <: Specification] private(entityContactPoint: ContactPoint, coordinator: ContactPoint)
  ...
  override def stateAfterMaxRetries: PartialFunction[(ParticipantState, ParticipantData), State] = {
    case (Init, _)     => goto(Abort) applying
      AbortReasonsAdded(NonEmptyList.of(SyncFailed(s"TransactionParticipant created but not asked to vote within $nextTimeoutAt")))
    case (Checking, _) => goto(Abort) applying
      AbortReasonsAdded(NonEmptyList.of(SyncFailed(s"TransactionParticipant asked to vote, but no response from entity within $nextTimeoutAt")))
  ...
  startWith(Init, ParticipantData(TransactionId(null), null, ParticipantId(null), NonEmptyList.of(SyncFailed(s"Uninitialized")), Set()))
    ...
    domainEvent match {
      case Initialized(id, request, participantId)     =>
        ParticipantData(id, request, participantId,
          NonEmptyList.of(SyncFailed(s"Sync `$request` for `$participantId` failed")), Set())
