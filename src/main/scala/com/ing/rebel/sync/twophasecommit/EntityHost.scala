...
import com.ing.rebel.sync.pathsensitive.{AllowCommandDecider, StaticCommandDecider, TwoPCCommandDecider}
...
import com.ing.rebel.{QuerySyncEvent, RebelCheck, RebelConditionCheck, RebelDomainEvent, RebelError, RebelState, RebelSuccess, RebelSyncEvent, SpecificationEvent}
import kamon.Kamon
...
//    Actor with
  ...
  Kamon.counter(s"create-$entityTypeName").increment()
...
//  val maxTransactionsInProgress = rebelConfig.sync.maxTransactionsInProgress
  ...
  /**
    * Tracks active transaction participants, should be removed when state is updated. Some for events, None for Queries. Option is some when ready for release.
    */
  ...
  def allowEvent(event: RDomainEvent): AllowCommandDecision = {
    ...
    val sampler = Kamon.buildSpan("CommandDecider").start()
    ...
    //    } else {
    // filter out queries
    val relevantTransactions: Seq[RDomainEvent] = activeTransactions.values.map(_.domainEvent).to[Seq].flatten
...
//    log.debug("Command decider took {} nanos", System.nanoTime() - startTime)
  ...
  val managerRelatedReceive: Receive = {
    ...
    case command: RebelCommand[S] =>
      ...
      val decision = allowEvent(dEvent)
//      log.debug("Finished allowEvent")
      decision match {
        case _:Accept       =>
...
//          log.debug("Created Initialize message: {}", initialize)
        ...
        case StaticReject          =>
          sender() ! EntityCommandFailed(dEvent, NonEmptyList.one(CommandNotAllowedInState(currentState, dEvent.specEvent)))
        case _:Delay        =>
          log.debug("{} not allowed by AllowCommandDecider, delaying", command)
          unavailable(command)
      }
  }
  ...
  val participantRelatedReceive: Receive = {
    ...
    case voteRequest@VoteRequest(tid, pid, event: RDomainEvent) =>
      allowEvent(event) match {
        ...
        case StaticReject          =>
          sender() ! VoteAbort(tid, pid, NonEmptyList.one(CommandNotAllowedInState(currentState, event.specEvent)))
        case DynamicReject(errors) =>
    ...
    case query: RebelQuery =>
      log.debug("Unknown query after id, not found in {}, or wel in {}", finishedTransactions, activeTransactions.keys)
  ...
  lazy val lockClash: CounterMetric = Kamon.counter("lockClash")
