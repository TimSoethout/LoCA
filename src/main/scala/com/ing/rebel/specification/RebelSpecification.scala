...
import com.ing.rebel.sync.twophasecommit.TwoPhaseCommit.{Initialize, SyncOperation}
...
//  type RCommand = RebelCommand[SpecEvent]
  ...
  def syncOperations(domainEvent: RDomainEvent)(data: RData): RebelCheck[Set[SyncOperation]]
  ...
  def nextStateGeneric(currentStateLabel: RState, command: SpecEvent): RebelCheck[RState] = {
    ...
    Validated.fromOption(label, CommandNotAllowedInState(currentStateLabel, command)).toValidatedNel
  }

  private def checkCondition(cond: => Boolean, failureMessage: RebelError): RebelConditionCheck =
    try {
      if (cond) RebelConditionCheck.success else Validated.invalidNel[RebelError, RebelSuccess](failureMessage)
    } catch {
      case exception: Exception => Validated.invalidNel[RebelError, RebelSuccess](GenericRebelError(exception))
    }
