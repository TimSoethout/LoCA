...
package object rebel {
  ...
  type RebelCheck[A] = ValidatedNel[RebelError, A]
  object RebelCheck {
    def success[A](a:A) : RebelCheck[A] = Validated.Valid(a)
   }
  type RebelErrors = NonEmptyList[RebelError]
  ...
  object RebelConditionCheck {
    ...
    def failure[A](error: RebelError): RebelCheck[A] = Validated.invalidNel(error)

    def failure[A](errors: RebelErrors): RebelCheck[A] = Invalid(errors)
  }
