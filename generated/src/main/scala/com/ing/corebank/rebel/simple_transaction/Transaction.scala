...
import com.ing.rebel.sync.twophasecommit.TwoPhaseCommit.{ParticipantId, SyncAction, SyncOperation}
...
trait TransactionLogic extends RebelSpecification[Transaction.type] {
  ...
  override def syncOperations(domainEvent: RDomainEvent)(data: RData): RebelCheck[Set[SyncOperation]] = {
    domainEvent.specEvent match {
      case Book(amount, from, to) =>
        RebelCheck.success(Set(
          //           Account(from)(RebelDomainEvent(Withdraw(amount)))
          SyncAction(ParticipantId(UniqueId(from.toString)), ShardingContactPoint(Account, from), RebelDomainEvent(Withdraw(amount))),
          SyncAction(ParticipantId(UniqueId(to.toString)), ShardingContactPoint(Account, to), RebelDomainEvent(Deposit(amount)))
        ))
    }
  }
